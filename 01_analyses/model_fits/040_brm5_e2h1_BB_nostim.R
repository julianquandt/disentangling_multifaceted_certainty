...
bf_brm5_e2h1_BB_nostim <- bf(confidence | trials(200) ~ basket_cat_f + basket_value_s + item_value_sd_s + basket_value_ex_s
  + (1 + basket_cat_f + basket_value_s + item_value_sd_s + basket_value_ex_s | subject))

brm5_e2h1_BB_nostim <- brm(bf_brm5_e2h1_BB_nostim,
  data = basket_conf_data,
  family = beta_binomial(),
  chains = 4, cores = 4, warmup = 2000, iter = 4000,
  backend = "cmdstanr", threads = threading(3),
  seed = 1,
  file = here::here("01_analyses/fitted_models/040_brm5_e2h1_bb_nostim.rds"),
