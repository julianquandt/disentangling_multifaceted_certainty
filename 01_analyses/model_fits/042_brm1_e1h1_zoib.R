...
bf1_e1h1_zoib <- bf(confidence_01 ~ confidence_mean_s + value_sd_s
                    + (1 + confidence_mean_s + value_sd_s | subject)
                    + (1 | stim_name),
                    phi ~ confidence_mean_s + value_sd_s,
                    zoi ~ confidence_mean_s + value_sd_s,
                    coi ~ confidence_mean_s + value_sd_s
  )

brm1_e1h1_zoib <- brm(bf1_e1h1_zoib,
                    data = basket_conf_data_prereg,
                    family = zero_one_inflated_beta(),
                    chains = 4, cores = 4, warmup = 2000, iter = 4000,
                    backend = "cmdstanr", threads = threading(2),
                    seed = 1,
                    file = here::here("01_analyses/fitted_models/042_brm1_e1h1_zoib.rds"),
