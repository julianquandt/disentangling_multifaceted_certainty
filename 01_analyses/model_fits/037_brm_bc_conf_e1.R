...
bf_bc_conf <- bf(confidence_mean ~  basket_confidence_cat_f+basket_diversity_cat_f 
           + (1 + basket_confidence_cat_f+basket_diversity_cat_f | subject))

brm_bc_conf <- brm(bf_bc_conf
           , data = basket_conf_data_prereg
           , family = gaussian()
           , chains = n_cores, cores = n_cores, warmup = 2000, iter = 4000
           , backend = "cmdstanr", threads = threading(2)
           , seed = 1
           , file = here::here("01_analyses/fitted_models/037_brm_bc_conf_e1.rds")
