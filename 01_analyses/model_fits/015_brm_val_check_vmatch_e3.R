...
bf_val_check_vmatch <- bf(basket_value_price ~ basket_cat_f 
           + (1 + basket_cat_f | subject_nr))


brm_val_exp3_eval <- brm(bf_val_check_vmatch
           , data = basket_conf_data_eval
           , family = gaussian()
           , chains = 4, cores = 4, warmup = 1000, iter = 2000
           , backend = "cmdstanr", threads = threading(3)
           , seed = 1
           , file = here::here("01_analyses/fitted_models/015_brm_val_check_vmatch_e3.rds")
