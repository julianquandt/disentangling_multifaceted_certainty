...
bf5_e3h1_gauss <- bf(confidence_01 ~ basket_cat_f*basket_matching_var_f + basket_value_s + value_sd_s  + basket_value_ex_s
           + (1 + basket_cat_f*basket_matching_var_f  + basket_value_s + value_sd_s  + basket_value_ex_s | subject_nr)
           + (1 | stim_name))

brm5_e3h1_gauss <- brm(bf5_e3h1_gauss
                    , data = basket_conf_data
                    , family = gaussian()
                    , chains = 4, cores = 4, warmup = 2000, iter = 4000
                    , backend = "cmdstanr", threads = threading(3)
                    , seed = 1
                    , file = here::here("01_analyses/fitted_models/045_brm5_e3h1_gauss.rds")
