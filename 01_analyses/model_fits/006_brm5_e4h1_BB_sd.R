...
bf_brm5_e4h1_BB_ex_sd <- bf(confidence | trials(200) ~  +  basket_cat_f
                      + (1 +  basket_cat_f | subject_nr))


brm5_e4h1_BB_sd <- brm(bf_brm5_e4h1_BB_ex_sd
                    , data = basket_conf_data_sd
                    , family = beta_binomial()
                    , chains = 4, cores = 4, warmup = 2000, iter = 4000
                    , backend = "cmdstanr", threads = threading(3)
                    , seed = 1
                    , file = here::here("01_analyses/fitted_models/006_brm5_e4h1_BB_sd.rds")
