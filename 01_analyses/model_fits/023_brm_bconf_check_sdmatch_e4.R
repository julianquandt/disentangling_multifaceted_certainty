...
bf_bconf_check <- bf(basket_rating_sd ~ basket_cat_f 
           + (1 + basket_cat_f | subject_nr))

brm_bconf_exp4_cons <- brm(bf_bconf_check
           , data = basket_conf_data_sd
           , family = gaussian()
           , chains = 4, cores = 4, warmup = 1000, iter = 2000
           , backend = "cmdstanr", threads = threading(3)
           , seed = 1
           , file = here::here("01_analyses/fitted_models/023_brm_bconf_check_sdmatch_e4.rds")
