...
bf_brm4_e1h1_BB <- bf(confidence | trials(200) ~  basket_value_s + basket_value_ex_s 
                          + (1 + basket_value_s + basket_value_ex_s | subject) 
                          + (1 | stim_name))

brm4_e1h1_BB <- brm(bf_brm4_e1h1_BB
                    , data = basket_conf_data_prereg
                    , family = beta_binomial()
                    , chains = n_cores, cores = n_cores, warmup = 1000, iter = 2000
                    , backend = "cmdstanr", threads = threading(2)
                    , seed = 1
                    , file = here::here("01_analyses/fitted_models/033_brm4_e1h1_BB.rds")
                    , file_refit = "on_change"
           )

cvstrat_brm4_BB <- kfold(brm4_e1h1_BB, K = 10, folds = basket_conf_data_prereg$fold_ex2basket, save_fits = TRUE)
if(!file.exists(here::here("01_analyses/fitted_models/036_cvstrat_brm4_BB_lite.rds"))){
  saveRDS(cvstrat_brm4_BB, here::here("01_analyses/fitted_models/036_cvstrat_brm4_BB_lite.rds"))
}