...
bf5_e3h1_zoib <- bf(confidence_01 ~ basket_cat_f*basket_matching_var_f  + basket_value_s + value_sd_s  + basket_value_s + basket_value_ex_s
                    + (1 + basket_cat_f*basket_matching_var_f  + basket_value_s + value_sd_s  + basket_value_s + basket_value_ex_s | subject_nr),
                    phi ~ basket_cat_f + (1 + basket_cat_f  | subject_nr),
                    zoi ~ basket_cat_f + (1 + basket_cat_f | subject_nr),
                    coi ~ basket_cat_f + (1 + basket_cat_f | subject_nr)    
)

brm5_e3h1_zoib <- brm(bf5_e3h1_zoib
                     , data = basket_conf_data
                    , family = zero_one_inflated_beta() 
                    , chains = 4, cores = 4, warmup = 2000, iter = 4000
                    , backend = "cmdstanr", threads = threading(3)
                    , seed = 1
                    , file = here::here("01_analyses/fitted_models/046_brm5_e3h1_zoib.rds")
