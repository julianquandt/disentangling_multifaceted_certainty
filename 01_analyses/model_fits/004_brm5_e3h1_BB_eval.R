...
basket_conf_data_eval <- subset(basket_conf_data, basket_matching_var == "evaluation")
basket_conf_data_eval$basket_value_s <- scale(basket_conf_data_eval$basket_value, scale = T)
basket_conf_data_eval$basket_value_ex_s <- scale(basket_conf_data_eval$basket_value_ex, scale = T)
basket_conf_data_eval$value_sd_s <- scale(basket_conf_data_eval$value_sd, scale = T)



bf_brm5_e3h1_BB_evalex <- bf(confidence | trials(200) ~ basket_cat_f + basket_value_s + value_sd_s  + basket_value_ex_s
                      + (1 +  basket_cat_f  + basket_value_s + value_sd_s  + basket_value_ex_s | subject_nr))



brm5_e3h1_BB_eval <- brm(bf_brm5_e3h1_BB_evalex
                    , data = basket_conf_data_eval
                    , family = beta_binomial()
                    , chains = 8, cores = 8, warmup = 2000, iter = 4000
                    , backend = "cmdstanr", threads = threading(3)
                    , seed = 1
                    , file = here::here("01_analyses/fitted_models/004_brm5_e3h1_BB_eval.rds")
