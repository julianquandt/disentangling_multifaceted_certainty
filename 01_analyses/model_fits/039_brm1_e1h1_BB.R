...
bf_brm1_e1h1_BB <- bf(confidence | trials(200) ~  confidence_mean_s + value_sd_s + (1 + confidence_mean_s + value_sd_s | subject))

brm1_e1h1_BB <- brm(bf_brm1_e1h1_BB
           , data = basket_conf_data_prereg
           , family = gaussian()
           , chains = n_cores, cores = n_cores, warmup = 2000, iter = 4000
           , backend = "cmdstanr", threads = threading(3)
           , seed = 1
           , file = here::here("01_analyses/fitted_models/039_brm1_e1h1_bb.rds")
