...
bf_brm3_e1h1_BB_cov <- bf(confidence | trials(200) ~  value_sd_s + basket_value_s + basket_value_ex_s + (1 + value_sd_s + basket_value_s + basket_value_ex_s | subject) + (1 | stim_name))

brm3_e1h1_BB_cov <- brm(bf_brm3_e1h1_BB_cov
                    , data = basket_conf_data_prereg
                    , family = beta_binomial2
                    , chains = n_cores, cores = n_cores, warmup = 1000, iter = 2000
                    , backend = "cmdstanr", threads = threading(2)
                    , seed = 1
                    , file = here::here("01_analyses/fitted_models/032_brm3_e1h1_BB_cov.rds")
                    , file_refit = "on_change"
           )

cvstrat_brm3_BB_cov <- kfold(brm3_e1h1_BB_cov, K = 10, folds = basket_conf_data_prereg$fold_ex2basket, save_fits = TRUE)
if(!file.exists(here::here("01_analyses/fitted_models/035_cvstrat_brm3_BB_cov_lite.rds"))){
  saveRDS(cvstrat_brm3_BB_cov, here::here("01_analyses/fitted_models/035_cvstrat_brm3_BB_cov_lite.rds"))
}
