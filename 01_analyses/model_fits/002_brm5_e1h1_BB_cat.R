...
bf_brm5_e1h1_BB_cat <- bf(confidence | trials(200) ~ basket_diversity_cat_f+basket_confidence_cat_f + basket_value_s + basket_value_ex_s
           + (1 + basket_diversity_cat_f+basket_confidence_cat_f+ basket_value_s + basket_value_ex_s | subject))

brm5_e1h1_BB_cat <- brm(bf_brm5_e1h1_BB_cat
                    , data = basket_conf_data_prereg_div_reverse
                    , family = beta_binomial()
                    , chains = 4, cores = 4, warmup = 2000, iter = 4000
                    , backend = "cmdstanr", threads = threading(3)
                    , seed = 1
                    , file = here::here("01_analyses/fitted_models/002_brm5_e1h1_bb_cat.rds")
