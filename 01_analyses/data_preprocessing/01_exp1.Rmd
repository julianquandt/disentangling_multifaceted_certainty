...
osf_RAW_datafolder <- osf_ls_files(osf_node, path = "RAW_DATA\\Exp1", n_max = Inf) # get data folder
...
basket_eval_data <- basket_eval_data[,which(names(basket_eval_data) %in% 
                                              c("rt", "response", "stim_name", 
                                                ...
                                                "basket_cat", 
                                                "basket_confidence_cat", 
                                                "basket_diversity_cat"))]
# rename response to evaluation
...
# Variable Explanations:
## RT: response time for each rating
## evaluation: basket-evaluation (0-200)
## stim_name: the file-path of the respective item that was presented
## block_name: low-calorie (LC) or high calorie (HC)
## subject:  the subject-key that identifies unique participants
## subject_nr: number of the subject-key that identifies unique participants
## counterbalance: which block was first?
## stimulus_cat: low-calorie (LC) or high calorie (HC); basically identical to block_name
## trial_nr: the number of the respective trial
## block: which block is the rating from 
## rep_nr: which repetition of the rating is this? (1-3) 
## item1_value: evaluation of the first item in basket
## item1_confidence: confidence-rating of the first item in basket
## item1_valueCat: which value-catogory is item 1 in the basket from based on the ordered evaluations of this particular participant (low/med/high)
## item1_confidenceCat: which confidence-category is item 1 in the basket from based on the ordered confidence-ratings of this particular participant (low/med/high)
## ...
## ...
## ...
## ...
...
## ...
## ...
## basket_cat: which category is this basket (Chigh_Dhigh = confidence high / diversity high | Chigh_Dlow = confidence high / diversity low | Clow_Dhigh = confidence low / diversity-high | Clow_Dlow = both low)
## basket_confidence_cat: same information as variable above but for confidence: is it a high confidence basket or low confidence basket (high/low) 
## basket_diversity_cat: same information as variable above but for diversity: is it a high diversity basket or low diversity basket (high/low) 
#####################################################################
...
varexplanation_basket_eval <- 
  c("response time for each rating", 
    ...
    "which value-catogory is item 1 in the basket from based on the ordered evaluations of this particular participant (low/med/high)",
    "which confidence-catogory is item 1 in the basket from based on the ordered confidence-ratings of this particular participant (low/med/high)",
    "valuation of the second item in basket ",
    "confidence-rating of the second item in basket",
    "which value-catogory is item 2 in the basket from based on the ordered evaluations of this particular participant (low/med/high)",
    "which confidence-catogory is item 2 in the basket from based on the ordered confidence-ratings of this particular participant (low/med/high)",
    "valuation of the third item in basket ",
    "confidence-rating of the third item in basket",
    "which value-catogory is item 3 in the basket from based on the ordered evaluations of this particular participant (low/med/high)",
    "which confidence-catogory is item 3 in the basket from based on the ordered confidence-ratings of this particular participant (low/med/high)",
    ...
    "which category is this basket (Chigh_Dhigh = confidence high / diversity high | Chigh_Dlow = confidence high / diversity low | Clow_Dhigh = confidence low / diversity-high | Clow_Dlow = both low)",
    "same information as variable above but for confidence: is it a high confidence basket or low confidence basket (high/low) ",
    "same information as variable above but for diversity: is it a high diversity basket or low diversity basket (high/low)"
    )
...
# Variable Explanations:
## RT: response time for each rating
## evaluation: basket-confidence rating (0-200)
## stim_name: the file-path of the respective item that was presented
## block_name: low-calorie (LC) or high calorie (HC)
## subject:  the subject-key that identifies unique participants
## subject_nr: number of the subject-key that identifies unique participants
...
```{r extract_choice_data}
# extract choice trials
choice_data <- subset(raw_data, response_cat == "choice")
# select relevant columns for choice data
choice_data <- choice_data[,which(names(choice_data) %in% 
                                    c("rt", "response", "subject",  
                                      "trial_nr", "block",
                                      "item_left", "item_right", "rating_left", 
                                      "rating_right", "confidence_left", 
                                      "confidence_right"))]

# rename response to chosen_side
names(choice_data)[which(names(choice_data) == "response")] <- "chosen_side"
#make new variable higher_chosen
choice_data$higher_chosen <- rep(NA)

# Variable Explanations:
## RT: response time for each choice
## chosen_side: choice (left or right) for each choice-pair
## subject: the subject-key that identifies unique participants
## trial_nr: the number of the respective trial
## block: which block is the rating from 
## item_left: which basket was presented left
## item_right: which basket was presented right
## rating_left: the evaluation rating of the left basket
## confidence_left: the confidence rating of the left basket
## rating_right: the evaluation rating of the right basket
## confidence_right: the confidence rating of the right basket
## higher_chosen: whether the higher-value item was chosen
...
for(i in 1:nrow(choice_data)){
  ...
  # remove file path and concatenate them to single string
  # items should already be sorted but sorting is applied gain to be sure
  choice_data$item_left[i] <- paste(sort(as.vector(
    sapply(strsplit(choice_data$item_left[i], ","), 
           function(x) gsub('.png','',basename(x))))), collapse = "")
  choice_data$item_right[i] <- paste(sort(as.vector(
    sapply(strsplit(choice_data$item_right[i], ","), 
           function(x) gsub('.png','',basename(x))))), collapse = "")
  
  if(choice_data$chosen_side[i] == "left"){
    if(choice_data$rating_left[i] > choice_data$rating_right[i]){
      choice_data$higher_chosen[i] <- 1
    } else {
      choice_data$higher_chosen[i] <- 0
    }
  } else {
    if(choice_data$rating_left[i] <= choice_data$rating_right[i]){
      choice_data$higher_chosen[i] <- 1
...
`r ncol(choice_data)` relevant variables were extracted:

<div class="table-container">
```{r choice_codebook, echo = F}
vars_choice <- colnames(choice_data)
varexplanation_choice <- c("response time for each choice", 
                           "choice (left or right) for each choice-pair", 
                 "the subject-key that identifies unique participants", 
                 "the number of the respective trial",
                 "which block is the rating from", 
                 "which basket was presented left", 
                 "which basket was presented right",
                 "the evaluation rating of the left basket", 
                 "the evaluation rating of the right basket",
                 "the confidence rating of the left basket", 
                 "the confidence rating of the right basket",
                 "whether the higher value item was chosen (1 = yes / 0 = no)")

tcb_choice<- make_table(data.frame(cbind(vars_choice, varexplanation_choice)))
tcb_choice<- set_header_labels(tcb_choice, vars_choice = "variable", 
                               varexplanation_choice = "explanation")
tcb_choice
```
...
<div class="table-container">
`r make_table(head(choice_data), ali = "right", aw = 2)`
</div>
...
stopifnot(nrow(single_eval_data)+nrow(single_conf_data)+
            nrow(basket_eval_data)+nrow(basket_conf_data)+
            nrow(choice_data) == nrow(raw_data))
```
...
```{r upload_to_osf, include = TRUE, output = FALSE, eval = FALSE}
### The evaluation of this is set to FALSE because the upload to OSF is not necessary for the reproduction
# mark upload folder
processed_data_dir <- osf_ls_files(osf_node)[which(osf_ls_files(osf_node)$name == "processed_data"),]

# single evaluations
write.csv(single_eval_data, "single_eval_data_exp1.csv")
osf_upload_file(osf_node = osf_node, remote_path = "single_eval_data_exp1.csv", 
                local_path = "single_eval_data_exp1.csv", conflicts = "overwrite",  
                local_file_remove = TRUE, osf_direct_dir = processed_data_dir)

# single confidence
write.csv(single_conf_data, "single_conf_data_exp1.csv")
osf_upload_file(osf_node = osf_node, remote_path = "single_conf_data_exp1.csv", 
                local_path = "single_conf_data_exp1.csv", conflicts = "overwrite",  
                local_file_remove = TRUE, osf_direct_dir = processed_data_dir)

# basket evaluations
write.csv(basket_eval_data, "basket_eval_data_exp1.csv")
osf_upload_file(osf_node = osf_node, remote_path = "basket_eval_data_exp1.csv", 
                local_path = "basket_eval_data_exp1.csv", conflicts = "overwrite",  
                local_file_remove = TRUE, osf_direct_dir = processed_data_dir)

# basket confidence
write.csv(basket_conf_data, "basket_conf_data_exp1.csv")
osf_upload_file(osf_node = osf_node, remote_path = "basket_conf_data_exp1.csv", 
                local_path = "basket_conf_data_exp1.csv", conflicts = "overwrite",  
                local_file_remove = TRUE, osf_direct_dir = processed_data_dir)
```
