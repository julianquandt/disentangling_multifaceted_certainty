---
title: "Experiment 3 - Data Processing"
...
if(run_local == TRUE){
  raw_data <- read.csv(here::here("01_analyses/99_RAW_DATA/02_exp3/data.csv"), stringsAsFactors = F)


  # give participants a number
  subject_nrs <- c(1:length(unique(raw_data$subject)))
  set.seed(pp_rnd_seed) # seed is only saved locally.
  subject_nrs <- sample(subject_nrs)
  for(i in 1:nrow(raw_data)){
    raw_data$subject_nr[i] <- subject_nrs[which(unique(raw_data$subject) == raw_data$subject[i])]
  }
  ...
  write.table(raw_data, here::here("01_analyses/99_RAW_DATA/02_exp3/data_anom.csv"), row.names = FALSE, sep = ",")

} else {
  osf_node <- osf_retrieve_node("9y38n") # retrieve OSF node
  osf_RAW_datafolder <- osf_ls_files(osf_node, path = "RAW_DATA\\Exp3", n_max = Inf) # get data folder
...
```{r extract_evalconfidence_data}
#extract food ratings
item_conf_data <- subset(raw_data, trial_type == "image-slider-response" & 
                             response_cat == "confidence" & stimulus_type == "item")


# select relevant columns for rating data of single items
item_conf_data <- item_conf_data[,which(names(item_conf_data) %in% 
                                              c("rt", "response", "stim_name", 
                                                 "subject_nr", 
                                                 "trial_nr"))]
# rename response to evaluation
names(item_conf_data)[which(names(item_conf_data) == "response")] <- 
  "confidence"
item_conf_data$confidence <- as.numeric(item_conf_data$confidence)

for (i in 1:nrow(item_conf_data)){
  item_conf_data$stim_name[i] <- gsub('.png','',
                                        basename(item_conf_data$stim_name[i]))
  item_conf_data$price_exact[i] <- as.numeric(sub(".*(\\d+{4}).*$", "\\1", item_conf_data$stim_name[i]))
  item_conf_data$price_rounded[i] <- plyr::round_any(item_conf_data$price_exact[i], 50) 
}
```
...
`r ncol(item_conf_data)` relevant variables were extracted:

<div class="table-container">

```{r single_confidence_codebook, echo = F}
vars_single_conf <- colnames(item_conf_data)
varexplanation_single_conf <- 
  c("response time for each confidence rating", 
    "confidence in the bid (0-200)", 
    "the respective item that was presented", 
    "the number of the respective trial", 
    "the subject-number that identifies unique participants",
    "the real price of the item presented",
...
<div class="table-container">
`r make_table(head(item_conf_data), ali = "right", aw = 2)`
</div>
...
stopifnot(nrow(item_eval_data)+nrow(item_conf_data)+
...
## single confidence
item_conf_data$r_ident <- paste(item_conf_data$subject, item_conf_data$stim_name, item_conf_data$trial_nr, sep = "-")
### test for identical length
stopifnot(length(unique(item_conf_data$r_ident)) == nrow(item_conf_data))
## basket evaluation
...
item_eval_data <- merge(item_eval_data, item_conf_data[, c("r_ident", "confidence")], by="r_ident")
item_conf_data <- merge(item_conf_data, item_eval_data[, c("r_ident", "evaluation")], by="r_ident")
...
for(i in 1:nrow(basket_conf_data)){
  basket_conf_data$value_sd[i] <- sd(c(basket_conf_data$item1_value[i]*50, basket_conf_data$item2_value[i]*50, basket_conf_data$item3_value[i]*50))
...
for(i in unique(item_conf_data$subject_nr)){
  tmp_dat <- subset(item_conf_data, subject_nr == i)
...
for(i in 1:nrow(item_conf_data)){
  item_conf_data$prereg_exclusion[i] <- 
    ifelse(item_conf_data$subject_nr[i] %in% pp_exclude_list, 1, 0)
}
...
```{r upload_to_osf, output = FALSE, eval = FALSE} 
 ...
 # single evaluations 
 write.csv(item_eval_data, "item_eval_data_exp3.csv") 
 osf_upload_file(osf_node = osf_node, remote_path = "item_eval_data_exp3.csv",  
                 local_path = "item_eval_data_exp3.csv", conflicts = "overwrite",   
                local_file_remove = TRUE, osf_direct_dir = processed_data_dir) 

 # single confidence
 write.csv(item_conf_data, "item_conf_data_exp3.csv") 
 osf_upload_file(osf_node = osf_node, remote_path = "item_conf_data_exp3.csv",  
                 local_path = "item_conf_data_exp3.csv", conflicts = "overwrite",   
                 local_file_remove = TRUE, osf_direct_dir = processed_data_dir)

 # basket evaluations 
 write.csv(basket_eval_data, "basket_eval_data_exp3.csv") 
 osf_upload_file(osf_node = osf_node, remote_path = "basket_eval_data_exp3.csv",  
                 local_path = "basket_eval_data_exp3.csv", conflicts = "overwrite",  
                 local_file_remove = TRUE, osf_direct_dir = processed_data_dir)

 # basket confidence 
 write.csv(basket_conf_data, "basket_conf_data_exp3.csv")
 osf_upload_file(osf_node = osf_node, remote_path = "basket_conf_data_exp3.csv",  
                 local_path = "basket_conf_data_exp3.csv", conflicts = "overwrite",   
                 local_file_remove = TRUE, osf_direct_dir = processed_data_dir) 
```
